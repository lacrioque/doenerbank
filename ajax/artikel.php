...
if($values['artikel']=='alle'){
	...
	$out = json_encode(array('geschlossen'=>'false', "artikel"=>$artikel));
	if($objBestellung->isGeschlossen()){$out = json_encode(array('geschlossen'=>'true', "artikel"=>$artikel)); }
	
} else {
	$geschlossen = $objBestellung->clean_articles();
	if($geschlossen['geschlossen']===true){
		$out = json_encode(array('success'=>true, 'geschlossen'=>true));
		die($out);
	} else {
				if($values['artikel']=='einige'){
