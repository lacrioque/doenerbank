var Artikel = {
    ...
    tpl: "<div class='span3 artikel-einzel'><p class='artikel-name'> {{ name }} </p> <p class='artikel-beschreibung'> {{ &beschreibung }}</p><p class='artikel-preis'> {{ &preis }} </p> <button class='btn btn-primary btn-default artikel-bestellen artikel-interaktion' data-artikelnummer='{{ art_id }}' id='artikel_{{ art_id }}'><span>Will ich</span><span class='glyphicon glyphicon-shopping-cart' aria-hidden='true'></span></button><button class='artikel-interaktion btn btn-primary btn-default artikel-abbestellen hidden' data-artikelnummer='{{ art_id }}' id='artikel_rm_{{ art_id }} '><span>Will ich nicht mehr</span><span class='glyphicon glyphicon-shopping-cart' aria-hidden='true'></span></button></div>",
